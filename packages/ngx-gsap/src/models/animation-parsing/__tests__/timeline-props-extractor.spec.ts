...
describe('TimelinePropsExtractor', () => {
  it('should extract from beginning', () => {
    const { timelineVars, cleanSequence } = new TimelinePropsExtractor('timeline@repeat=3,yoyo=true;to:x:100:>').extract();
    expect(timelineVars).toEqual({ repeat: 3, yoyo: true });
    expect(cleanSequence).toBe('to:x:100:>');
  });

  it('should extract from middle', () => {
    const { timelineVars, cleanSequence } = new TimelinePropsExtractor('to:x:100:>;timeline@repeat=2;to:y:50:>').extract();
    expect(timelineVars).toEqual({ repeat: 2 });
    expect(cleanSequence).toBe('to:x:100:>;;to:y:50:>');
  });

  it('should extract from end', () => {
    const { timelineVars, cleanSequence } = new TimelinePropsExtractor('to:x:100:>;to:y:50:>;timeline@repeat=4').extract();
    expect(timelineVars).toEqual({ repeat: 4 });
    expect(cleanSequence).toBe('to:x:100:>;to:y:50:>');
  });

  it('should return empty vars when no timeline@', () => {
    const { timelineVars, cleanSequence } = new TimelinePropsExtractor('to:x:100:>;to:y:50:>').extract();
